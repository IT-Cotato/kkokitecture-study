...
- 채팅 서비스와의 접속에는 keep-alive 헤더를 사용하면 효율적
    - 클라이언트와 서버 사이의 연결을 끊지 않고 계속 유지할 수 있어서다.
...
![image](images/chapter12-01.png)


## 롱 폴링

![image](images/chapter12-02.png)
...
온라인 상태의 클라이언트로 하여금 주기적으로 박동 이벤트를 접속상태 서버로 보내도록 하고, 마지막 이벤트를 받은 지 x 초 이내에 또 다른 박동 이벤트 메시지를 받으면 해당 사용자의 접속 상태를 계속 온라인으로 유지하는 것이다.

![image](images/chapter12-13.png)
...
그룹 크기가 더 커지면 이런 식으로 접속상태 변화를 알려서는 비용이나 시간이 많이 들게되므로 좋지 않다.
