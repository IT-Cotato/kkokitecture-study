...
- 비용을 절감한다. 추가 요청에 대한 처리를 제한하면 서버를 많이 두지 않아도 되고, 우선순위가 높은 API에 더 많은 자원을 할당할 수 있다. 제3자(third-party) API에 사용료를 지불하고 있는 회사들에게는 아주 중요하다.
...
- 클라이언트 측에 둔다면: 일반적으로 클라이언트는 처리율 제한을 안정적으로 걸 수 있는 장소가 못된다. 클라이언트 요청은 쉽게 위변조가 가능해서다. 모든 클라이언트의 구현을 통제하는 것도 어려울 수 있다.
  - 클라이언트의 JS 코드를 수정해서 무력화 할 수도 있다.
  - Postman이나 Curl로 직접 서버에 요청을 보낼 수도 있다.
  - 웹 브라우저, iOS 앱, 안드로이드 앱 등 각각에 처리율 제한 로직을 심는 것도 일이지만, 업데이트를 안한 사용자가 있다면 그 곳은 처리율 제한 장치가 없다.

- 서버 측에 둔다면: 아래 그림은 서버 측에 처리율 제한 장치를 두는 한 가지 방법을 보여준다.

![](images/Pasted_image_20260404174736.png)

다른 방법도 있다. 처리율 제한 장치를 API 서버에 두는 대신, 처리율 제한 미들웨어(middleware)를 만들어 해당 미들웨어로 하여금 API 서버로 가는 요청을 통제하도록 하는 것이다.

- 미들웨어: 서로 다른 두 애플리케이션이나 시스템 사이에서 **다리 역할**을 하며 데이터나 서비스를 주고받을 수 있게 도와주는 소프트웨어를 통칭한다.

![](images/Pasted_image_20260404174838.png)

![](images/Pasted_image_20260404215451.png)

예제의 API 서버의 처리율이 초당 2개의 요청으로 제한도니 상황에서, 클라이언트가 3번째 요청을 앞의 두 요청과 같은 초 범위 내에서 전송하였다고 해 보자.

- 앞서 두 요청은 API 서버로 전송될 것이다.
- 하지만 세 번째 요청은 처리율 제한 미들웨어에 의해 가로막히고 클라이언트는 HTTP 상태 코드 429(Too many requests)가 반환된다.

폭넓게 채택된 기술인 클라우드 마이크로서비스의 경우, 처리율 제한 장치는 보통 API 게이트웨이(gateway)라 불리는 컴포넌트에 구현된다.
...
- 일단은 API 게이트웨이가 처리율 제한을 지원하는 미들웨어라는 점만 기억하자.
...
- 정답은 없다.
  - 회사의 현재 기술 스택(technology stack)이나 엔지니어링 인력, 우선순위, 목표에 따라 달라질 수 있기 때문이다.
...
![](images/Pasted_image_20260404221835.png)
...
![](images/Pasted_image_20260404222401.png)
...
- 매초마다 열리는 윈도에 3개 이상의 요청이 밀료오면 초과분은 버려진다.

![](images/Pasted_image_20260404222945.png)

이 알고리즘의 가장 큰 문제는 윈도의 경계 부근에 순간적으로 많은 트래픽이 집중될 경우 윈도에 할당된 양보다 더 많은 요청이 처리될 수 있다는 것이다.

![](images/Pasted_image_20260404223654.png)

위의 그림은 분당 최대 5개의 요청만을 허용하는 시스템이다.

- 카운터는 매분마다 초기화된다.
- 2:00:00와 2:01:00 사이에 다섯 개의 요청이 들어왔고, 2:01:00과 2:02:00 사이에 또 다섯 개의 요청이 들어왔다.
- 윈도 위치를 조금 옮겨 2:00:30부터 2:01:30까지의 1분 동안을 살펴보면, 이 1분 동안 시스템이 처리한 요청은 10개이다. 허용 한도의 2배인 것이다.
...
![](images/Pasted_image_20260404224152.png)

위 그림의 처리율 제한기는 분당 최대 2회의 요청만을 처리하도록 설정되었다.
...
- 새로운 요청이 1:01:40에 도착한다. \[1:00:40, 1:01:40) 범위 안에 있는 요청은 1분 윈도 안에 있는 요청이지만, 1:00:40 이전의 타임스탬프는 전부 만료 된 값이다. 따라서 두 개의 만료된 타임스탬프 1:00:01과 1:00:30을 로그에서 삭제한다. 삭제 직후 로그의 크기는 2이다. 따라서 1:01:40의 신규 요청은 시스템에 전달된다.
...
- 이 알고리즘은 다량의 메모리를 사용하는데, 거부된 요청의 타임스탬프도 보관하기 때무니다.

---

### 이동 윈도 카운터 알고리즘

이동 윈도 카운터(sliding window counter) 알고리즘은 고정 윈도 카운터 알고리즘과 이동 윈도 로깅 알고리즘을 결합한 것이다. 이 알고리즘을 구현하는 데는 두 가지 접근법이 사용될 수 있는데, 이번 절에서는 그중 하나만 설명하고 다른 하나에 대해서는 참고문헌을 언급하는 것으로 설명을 대신하겠다.

![](images/Pasted_image_20260404224929.png)
...
- 이 공식에 따르면 현재 윈도에 들어 있는 요청은 3 + 5 x 70% = 6.5개다. 본 예제에서는 내림하여 쓰면 그 값은 6이다.
...
- 직전 시간대에 도착한 요청이 균등하게 분포되어 있다고 가정한 상태에서 추정치를 계산하기 때문에 다소 느슨하다. 하지만 이 문제는 생각만큼 심각한 것은 아닌데, 클라우드플레어(cloudflare)가 실시했던 실험에 따르면 40억 개의 요청 가운데 시스템의 실제 상태와 맞지 않게 허용되거나 버려진 요청은 0.003%에 불과하였다.
...
- 레디스(Redis)는 처리율 제한 장치를 구현할 때 자주 사용되는 메모리 기반 저장장치로서, INCR과 EXPIRE의 두 가지 명령어를 지원한다.
  - INCR: 메모리에 저장된 카운터의 값을 1만큼 증가시킨다.
  - EXPIRE: 카운터에 타임아웃 값을 설정한다. 설정된 시간이 지나면 카운터는 자동으로 삭제된다.

![](images/Pasted_image_20260404230119.png)
...
위 그림의 개략적 설계를 봐서는 다음과 같은 사항은 알 수가 없다.
...
이 규칙은 클라이언트가 분당 5회 이상 로그인 할 수 없도록 제한하고 있다. 이런 규칙들은 보통 설정 파일 형태로 디스크에 저장된다.

---

## 처리율 한도 초과 트래픽의 처리

어떤 요청이 한도 제한에 걸리면 API는 HTTP 429 응답(too many requests)을 클라이언트에게 보낸다. 경우에 따라서는 한도 제한에 걸린 메시지를 나중에 처리하기 위해 큐에 보관할 수도 있다. 예를 들어 어떤 주문이 시스템 과부하 때문에 한도 제한에 걸렸다고 해 보자. 해당 주문들은 보관했다가 나중에 처리할 수도 있을 것이다.

---

### 처리율 제한 장치가 사용하는 HTTP 헤더

클라이언트가 자기 요청이 처리율 제한에 걸리고 있는지, 처리율 제한에 걸리기까지 얼마나 많은 요청을 보낼 수 있는지 알 수 있는 방법이 HTTP 응답 헤더(response header)에 있다. 이번 장에서 설계하는 처리율 제한 장치는 다음의 HTTP 헤더를 클라이언트에게 보낸다.
...
- 클라이언트가 요청을 서버로 보내면 요청은 먼저 처리율 제한 미들웨어에 도달한다.
...
![](images/Pasted_image_20260404231626.png)
...
![](images/Pasted_image_20260404235000.png)

경쟁 조건 문제를 해결하는 가장 널리 알려진 해결책은 락(lock)이다.

- 하지만 락은 시스템의 성능을 상당히 떨어뜨린다는 문제가 있다.
- 위 설계의 경우에는 락 대신 쓸 수 있는 해결책이 두 가지 있는데, 하나는 루아 스크립트(lua script)이고 다른 하나는 정렬 집합(sorted set)이라 불리는 레디스 자료구조를 쓰는 것이다.
...
![](images/Pasted_image_20260404235246.png)

- 위 그림에서 웹 계층은 무상태(stateless)이므로 클라이언트는 다음 요청을 오른쪽 그림처럼 각기 다른 제한 장치로 보내게 될 수 있다.
- 이때 동기화를 하지 않는다면 제한 장치 1은 클라이언트 2에 대해서는 아무것도 모르므로 처리율 제한을 올바르게 수행할 수 없을 것이다.
- 이에 대한 한 가지 해결책은 고정 세션(sticky session을 활용하여 같은 클라이언트로부터의 요청은 항상 같은 처리율 제한 장치로 보낼 수 있도록 하는 것이다.
  - 규모면에서 확장 가능하지도 않고 유연하지도 않기 때문에 추천하고 싶지 않다.
- 더 나은 해결책은 레디스와 같은 중앙 집중형 데이터 저장소를 쓰는 것이다.

![](images/Pasted_image_20260404235520.png)
...
예를 들어, 처리율 제한 규칙이 너무 빡빡하게 설정되었다면 많은 유효 요청이 처리되지 못하고 버려질 것이다. -> 규칙을 다소 완화할 필요가 있다.

---

# 4단계 마무리

다음 같은 부분들을 언급해보면 도움이 될 것이다.

- 경성(hard) 또는 연성(soft) 처리율 제한
  - 경성 처리율 제한: 요청의 개수는 임계치를 절대 넘어설 수 없다.
  - 연성 처리율 제한: 요청 개수는 잠시 동안은 임계치를 넘어설 수 있다.
- 다양한 계층에서의 처리율 제한
  - 이번 장에서는 애플리케이션 계층에서의 처리율 제한에 대해서만 살펴보았다. 하지만 다른 계층에서도 처리율 제한이 가능하다.
